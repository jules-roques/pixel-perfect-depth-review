...
def cat_dict(
    ...
    return {
        k: torch.cat([item[k] for item in dict_list], dim)
        for k in dict_list[0]
        if isinstance(dict_list[0][k], Tensor) and k not in reserved_keys
    }
...
def chunkify(
    ...
    **kwargs,
    ...
    assert all(
        [
            total_chunk_size == chunk_tensors[i].shape[chunk_dim]
            for i in range(1, len(chunk_tensors))
        ]
    )
    return cat_func(
        [
            func(
                *[
                    chunk_tensor[slice_func(i, chunk_dim, chunk_size)]
                    for chunk_tensor in chunk_tensors
                ],
                **kwargs,
            )
            for i in range(0, total_chunk_size, chunk_size)
        ],
        chunk_dim,
    )
    ...
    for i in range(0, chunk_size, chunk_num):
        ret = func(
            *[chunk_tensor[:, i : i + chunk_num] for chunk_tensor in chunk_tensors],
            **kwargs,
        )
