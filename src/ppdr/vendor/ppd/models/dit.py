...
class DiTBlock(nn.Module):
    ...
    def __init__(
        self, hidden_size, num_heads, mlp_ratio=4.0, rope=None, **block_kwargs
    ):
        super().__init__()
    ...
    def forward(self, x, c, pos=None):
        ...
        x = x + gate_msa.unsqueeze(1) * self.attn(
            modulate(self.norm1(x), shift_msa, scale_msa), pos=pos
        )
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.x_embedder = PatchEmbed(
            patch_size=patch_size * 2, in_chans=in_channels, embed_dim=hidden_size
        )
        self.t_embedder = TimestepEmbedder(hidden_size)

        self.blocks = nn.ModuleList(
            [
                DiTBlock(hidden_size, num_heads, mlp_ratio=mlp_ratio, rope=self.rope)
