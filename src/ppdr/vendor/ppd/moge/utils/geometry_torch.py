...
def gaussian_blur_2d(
    input: torch.Tensor, kernel_size: int, sigma: float
) -> torch.Tensor:
    kernel = torch.exp(
        -(
            torch.arange(
...
def angle_diff_vec3(v1: torch.Tensor, v2: torch.Tensor, eps: float = 1e-12):
    return torch.atan2(
        torch.cross(v1, v2, dim=-1).norm(dim=-1) + eps, (v1 * v2).sum(dim=-1)
    )
...
def point_map_to_depth_legacy(points: torch.Tensor):
    ...
    M = A.transpose(-2, -1) @ A
    solution = (
        torch.inverse(M + 1e-6 * torch.eye(2).to(A))
        @ (A.transpose(-2, -1) @ b[..., None])
    ).squeeze(-1)
...
def mask_aware_nearest_resize(
    ...
    device = mask.device
    filter_h_f, filter_w_f = (
        max(1, height / target_height),
        max(1, width / target_width),
    )
    ...
    indices = torch.arange(height * width, dtype=torch.long, device=device).reshape(
        height, width
    )
    ...
    padded_mask[..., padding_h : padding_h + height, padding_w : padding_w + width] = (
        mask
    )
    ...
    padded_indices[padding_h : padding_h + height, padding_w : padding_w + width] = (
        indices
    )
    ...
    A = torch.stack(
        [
            *torch.meshgrid(x, x, indexing="xy"),
            torch.ones((kernel_size, kernel_size), device=device, dtype=dtype),
        ],
        dim=-1,
    ).reshape(kernel_size**2, 3)  # [kernel_size ** 2, 3]
    A = mask_window[..., None] * A
    I = torch.eye(3, device=device, dtype=dtype)

    affine_disp_window = (
        disp_window[..., None, :] @ A @ torch.inverse(A.mT @ A + 1e-5 * I) @ A.mT
    ).clamp_min(1e-12)[..., 0, :]  # [..., H, W, kernel_size ** 2]
    diff = torch.where(
        mask_window,
        torch.maximum(affine_disp_window, disp_window)
        / torch.minimum(affine_disp_window, disp_window)
        - 1,
        0,
    )
    ...
    for _ in range(iterations):
        ...
        if filter == "min":
            input = torch.where(
                mask,
                input,
                torch.where(mask_window, input_window, torch.inf)
                .min(dim=(-2, -1))
                .values,
            )
        elif filter == "max":
            ...
            input = torch.where(
                mask,
                input,
                torch.where(mask_window, input_window, torch.nan)
                .flatten(-2)
                .nanmedian(dim=-1)
                .values,
            )
...
def refine_depth_with_normal(
    ...
    duv = torch.stack(
        torch.meshgrid(
            torch.linspace(
                -radius / width, radius / width, kernel_size, device=device, dtype=dtype
            ),
            torch.linspace(
                -radius / height,
                radius / height,
    ...
    log_depth_diff = (
        utils3d.torch.sliding_window_2d(
    ...
    laplacian = (
        ...
        * (
            (
                utils3d.torch.sliding_window_2d(
                    grad, window_size=kernel_size, stride=1, dim=(-3, -2)
                )
    ...
    for _ in range(iterations):
        log_depth_refine[..., radius:-radius, radius:-radius] = 0.1 * log_depth_refine[
