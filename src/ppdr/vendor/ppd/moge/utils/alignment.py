...
def scatter_min(
    ...
    ).scatter_reduce(dim=dim, index=index, src=src, reduce="amin", include_self=False)
...
def align_depth_affine(
    ...
    batch_size = math.prod(batch_shape)
    depth_src, depth_tgt, weight = (
        depth_src.reshape(batch_size, n),
        depth_tgt.reshape(batch_size, n),
    ...
    with torch.no_grad():
        ...
        depth_src_anchored = (
            depth_src[anchors_where_batch, :] - depth_src_anchor[..., None]
        )  # (anchors, n)
        depth_tgt_anchored = (
            depth_tgt[anchors_where_batch, :] - depth_tgt_anchor[..., None]
        )  # (anchors, n)
        weight_anchored = weight[anchors_where_batch, :]  # (anchors, n)

        scale, loss, index = align(
            depth_src_anchored, depth_tgt_anchored, weight_anchored, trunc
        )  # (anchors)
    ...
    tgt_1, src_1 = (
        torch.gather(depth_tgt, dim=1, index=index_1[..., None]).squeeze(-1),
        torch.gather(depth_src, dim=1, index=index_1[..., None]).squeeze(-1),
    )
    tgt_2, src_2 = (
        torch.gather(depth_tgt, dim=1, index=index_2[..., None]).squeeze(-1),
        torch.gather(depth_src, dim=1, index=index_2[..., None]).squeeze(-1),
    )
...
def align_depth_affine_irls(
    ...
    for i in range(max_iter):
        beta = (x.transpose(-1, -2) @ (w * y)) @ (
            x.transpose(-1, -2) @ (w[..., None] * x)
        ).inverse().transpose(-2, -1)
...
def align_points_scale_z_shift(
    ...
    with torch.no_grad():
        ...
        points_src_anchor = torch.stack(
            [zeros, zeros, points_src[anchor_where_batch, anchor_where_n, 2]], dim=-1
        )  # (anchors, 3)
        points_tgt_anchor = torch.stack(
            [zeros, zeros, points_tgt[anchor_where_batch, anchor_where_n, 2]], dim=-1
        )  # (anchors, 3)
        ...
        weight_anchored = weight[anchor_where_batch, :, None].expand(
            -1, -1, 3
        )  # (anchors, n, 3)
    ...
    points_tgt_00z, points_src_00z = (
        torch.stack([zeros, zeros, points_tgt[..., 2]], dim=-1),
        torch.stack([zeros, zeros, points_src[..., 2]], dim=-1),
    )
    tgt_1, src_1 = (
        torch.gather(
            points_tgt_00z.flatten(-2), dim=1, index=index_1[..., None]
        ).squeeze(-1),
        torch.gather(
            points_src_00z.flatten(-2), dim=1, index=index_1[..., None]
        ).squeeze(-1),
    )
    tgt_2, src_2 = (
        torch.gather(points_tgt.flatten(-2), dim=1, index=index_2[..., None]).squeeze(
            -1
        ),
        torch.gather(points_src.flatten(-2), dim=1, index=index_2[..., None]).squeeze(
            -1
        ),
    )
    ...
    shift = torch.gather(
        points_tgt_00z, dim=1, index=(index_1 // 3)[..., None, None].expand(-1, -1, 3)
    ).squeeze(-2) - scale[..., None] * torch.gather(
        points_src_00z, dim=1, index=(index_1 // 3)[..., None, None].expand(-1, -1, 3)
    ).squeeze(-2)
...
def align_points_scale_xyz_shift(
    ...
    with torch.no_grad():
        points_src_anchor = points_src[
            anchor_where_batch, anchor_where_n
        ]  # (anchors, 3)
        points_tgt_anchor = points_tgt[
            anchor_where_batch, anchor_where_n
        ]  # (anchors, 3)
    ...
    src_1, tgt_1 = (
        torch.gather(points_src.flatten(-2), dim=1, index=index_1[..., None]).squeeze(
            -1
        ),
        torch.gather(points_tgt.flatten(-2), dim=1, index=index_1[..., None]).squeeze(
            -1
        ),
    )
    src_2, tgt_2 = (
...
def align_points_xyz_shift(
    ...
    shift, _, _ = align(
        torch.ones_like(points_src).swapaxes(-2, -1),
        (points_tgt - points_src).swapaxes(-2, -1),
        weight[..., None, :],
        trunc,
    )
