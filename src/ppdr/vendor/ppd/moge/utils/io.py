...
def write_image(
    path: Union[str, os.PathLike, IO], image: np.ndarray, quality: int = 95
):
    """
...
def write_depth(
    ...
    depth = 1 + np.round(
        (
            np.log(np.nan_to_num(depth, nan=0).clip(near, far) / near)
            / np.log(far / near)
        ).clip(0, 1)
        * 65533
    ).astype(np.uint16)  # 1~65534
    depth[mask_nan] = 0
...
def write_segmentation(
    ...
    """
    assert mask.dtype == np.uint8 or mask.dtype == np.uint16, (
        f"Unsupported dtype {mask.dtype}"
    )
...
def write_normal(
    path: Union[str, os.PathLike, IO], normal: np.ndarray, compression_level: int = 7
) -> np.ndarray:
    """
