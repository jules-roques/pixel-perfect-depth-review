...
def recover_focal_shift_numpy(
    ...
    if mask is None:
        ...
        uv_lr = cv2.resize(uv, downsample_size, interpolation=cv2.INTER_LINEAR).reshape(
            -1, 2
        )
    else:
...
def mask_aware_nearest_resize_numpy(
    ...
    Union[np.ndarray, Tuple[np.ndarray, ...], None], np.ndarray, Tuple[np.ndarray, ...]
]:
    """
    ...
    target_window = np.round(target_lefttop).astype(np.int32) + np.array(
...
def mask_aware_area_resize_numpy(
    image: np.ndarray, mask: np.ndarray, target_width: int, target_height: int
    ...
    target_window = np.floor(target_lefttop).astype(np.int32) + np.array(
        (padding_w, padding_h), dtype=np.int32
    )
    ...
    target_window_area = (target_window_bottomright - target_window_lefttop).clip(
        0, None
    )
    ...
    target_window_image = image.reshape(*image.shape[:-3], height * width, -1)[
...
def depth_of_field(
    ...
    for radius in range(max_blur_radius + 1):
        dialted_blur_radii = np.clip(
            abs(dilated_disp[radius] - focus_disp) * max_blur_radius, 0, max_blur_radius
        ).astype(np.int32)
        mask = (
            (dialted_blur_radii >= radius)
