...
class ConcurrentNode(Node):
    ...
    def start(self):
        if self.running_as == "thread":
            ...
            job = Thread(
                target=self._loop_fn, args=(self.input, self.output, terminate_flag)
            )
...
class Provider(ConcurrentNode):
    ...
    def __init__(
        self, running_as: Literal["thread", "process"], out_buffer_size: int = 1
    ) -> None:
...
class ProviderFunction(Provider):
    def __init__(
        self, fn: Callable, running_as: "thread", out_buffer_size: int = 1
    ) -> None:
...
class Graph(Node):
    ...
    def link(
        self, src: Union[Node, Tuple[Node, str]], dst: Union[Node, Tuple[Node, str]]
    ):
        """
