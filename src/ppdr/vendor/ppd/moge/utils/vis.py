...
def colorize_disparity(
    ...
    if normalize:
        min_disp, max_disp = (
            np.nanquantile(disparity, 0.001),
            np.nanquantile(disparity, 0.999),
        )
...
def colorize_error_map(
    ...
    vmin, vmax = (
        ...
        else (np.nanmin(error_map), np.nanmax(error_map))
    )
    ...
    colorized_error_map = np.ascontiguousarray(
        (colorized_error_map.clip(0, 1) * 255).astype(np.uint8)
    )
