...
def generate_plots(results_json_path: str, output_path: str = "results.png"):
    ...
    for bar, val in zip(bars, avg_times, strict=True):
        ax.text(
    ...
    ax.set_title("Average Edge-Aware Chamfer Distance", fontsize=14, fontweight="bold")
    ax.spines[["top", "right"]].set_visible(False)
    for bar, val in zip(bars, avg_chamfer, strict=True):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height(),
            f"{val:.4f}",
